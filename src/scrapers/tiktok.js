...
export async function scrapeTikTok(url) {
  ...
  try {
    ...
    if (_ttSource === "snaptik") {
      ...
      // SnapTik's API sometimes exposes a separate HD field alongside the
      // standard one; prefer it and label accordingly when present.
      const hdUrl =
        info.hdplay ||
        info.hdDownloadUrl ||
        info.hd_url ||
        ...
        info.download_hd;
      if (hdUrl) {
        downloads.push({ type: "1080p", url: hdUrl });
      }
      if (info.downloadUrl && info.downloadUrl !== hdUrl) {
      ...
      if (isSlideshow) {
        ...
        while ((anchorMatch = anchorTagRegex.exec(html)) !== null) {
          ...
          if (isVideo) {
            ...
            // Prefer the actual resolution TikTokio printed on the button
            // (e.g. "480p", "540p", "720p", "1080p") over a guessed tier,
            // so every resolution the source actually offers shows up.
            const resMatch = innerText.match(/(\d{3,4})\s*p\b/i);
            let label;
            if (resMatch) {
              label = `${resMatch[1]}p`;
              if (downloads.some((d) => d.type === label)) {
                label = `${label} (${videoCount})`;
              }
            } else if (isHd) {
