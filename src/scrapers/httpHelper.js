...
export async function scraperFetch(options, serverName = "Server") {
  ...
  if (window.MoriShareBridge?.httpRequestAsync) {
    ...
    // Browser fallback (plain web deployment, e.g. Vercel): route through
    // our own same-origin /api/proxy so the request happens server-to-server
    // and never hits the browser's CORS restrictions.
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), getRequestTimeout());

    let proxyRes;
    try {
      proxyRes = await fetch("/api/proxy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          url: options.url,
    ...
    if (proxyJson.encoding === "base64" && options.responseType === "arraybuffer") {
