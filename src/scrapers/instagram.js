  ...
  try {
    ...
    if (htmlContent) {
      ...
      const addLink = (href, titleAttr, textContent, thumb) => {
        ...
        const combinedText = `${titleAttr || ""} ${textContent || ""}`.trim();
        const isImage = getIsImageFromUrlOrText(href, combinedText);
        ...
        // Surface any resolution/quality mentioned on the button itself
        // (e.g. "Download HD 1080p", "720p") so multiple options are
        // distinguishable instead of all showing as plain "MP4".
        const resMatch = combinedText.match(/(\d{3,4})\s*p\b/i);
        const qualityLabel = resMatch
          ? `${resMatch[1]}p`
          : /\bhd\b/i.test(combinedText)
            ? "HD"
            : /\bsd\b/i.test(combinedText)
              ? "SD"
              : undefined;
      ...
      targets.forEach((item) => {
        ...
        options.forEach((opt) => {
          ...
          downloadsMap.set(key, {
            url: val,
            type: isImage ? "PHOTO" : "MP4",
            quality: qualityLabel || undefined,
            thumbnail: thumb,
          });
        });
      });
