...
// Pinterest's CDN serves the same image at multiple sizes by folder name
// (e.g. /736x/, /564x/, /236x/). Swapping that folder for /originals/
// returns the full, un-resized version of the same image.
function upgradeToOriginal(pinimgUrl) {
  if (!pinimgUrl || !pinimgUrl.includes("i.pinimg.com")) return pinimgUrl;
  return pinimgUrl.replace(/\/(?:\d+x\d*|\d+x)\//, "/originals/");
}
  ...
  try {
    ...
    try {
      ...
      if (pageRes.data && typeof pageRes.data === "string") {
        ...
        const filteredImages = rawImageMatches
          .filter((u) => !isSiteAsset(u))
          .map(upgradeToOriginal);
